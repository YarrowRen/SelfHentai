...
class TranslationService:
    ...
    def __init__(self):
        ...
        # 从配置文件读取配置，支持多种LLM服务商
        self.base_url = settings.TRANSLATION_BASE_URL
        self.model_name = settings.TRANSLATION_MODEL
        self.api_key_env = settings.TRANSLATION_API_KEY_ENV
...
请翻译以下日文文本："""
    ...
    def initialize(self):
        ...
        try:
            # 获取 API Key（支持多种服务商的API Key环境变量名）
            api_key = getattr(settings, self.api_key_env)
            if not api_key:
                logger.error(f"未找到 {self.api_key_env} 配置")
                return False
            ...
            logger.info(f"翻译服务初始化成功 - 模型: {self.model_name}, API: {self.base_url}")
            return True
    ...
    def translate_text(self, japanese_text: str, target_language: str = "zh") -> dict:
        ...
        if not self.is_initialized:
            if not self.initialize():
                return {"success": False, "translation": "", "error": "翻译服务初始化失败"}

        if not japanese_text or not japanese_text.strip():
            return {"success": False, "translation": "", "error": "输入文本为空"}

        if target_language != "zh":
            return {"success": False, "translation": "", "error": f"暂不支持目标语言: {target_language}，目前只支持中文 (zh)"}

        try:
            logger.info(f"开始翻译日文文本，长度: {len(japanese_text)}")

            # 构建翻译请求
            messages = [{"role": "user", "content": f"{self.translation_prompt}\n\n{japanese_text}"}]

            # 调用 API
            response = self.client.chat.completions.create(
                model=self.model_name,
                messages=messages,
                temperature=0.6,  # 较低的温度确保翻译的一致性
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "api_key_available": bool(getattr(settings, self.api_key_env, None)),
